...
def parse_simulation_file(file_path):
    ...
    for motetype in root.findall(".//motetype"):
        for mote in motetype.findall(".//mote"):
            mote_info = {}
            position = mote.find(".//interface_config/pos")
            mote_id = mote.find(".//interface_config/id")
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            if match:
                ...
                if "Tx" in info:
                    message_id = info.split()[1].strip("'")
                    origin_addr = info.split()[-1].strip("'")
                    message_key = f"{message_id}|{origin_addr}"
                    if message_key not in messages:
                        messages[message_key] = []
                    messages[message_key].append((time, node_id, 'transmit'))
                    ...
                    if message_key not in messages:
                        messages[message_key] = []
                    messages[message_key].append((time, node_id, 'broadcast'))
